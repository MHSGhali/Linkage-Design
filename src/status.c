...
int status_alpha(const StatusMessage *msg, unsigned int now_ms) {
    ...
    unsigned int age = now_ms - msg->stamp_ms;
    if (age >= STATUS_FADE_MS) return 0;
    unsigned int solid = STATUS_FADE_MS - STATUS_FADE_TAIL_MS;
    if (age <= solid) return 255;
    unsigned int into_tail = age - solid;
...
void status_level_color(StatusLevel level, unsigned char *r, unsigned char *g, unsigned char *b) {
    switch (level) {
    case STATUS_WARN:  *r = 245; *g = 195; *b = 90;  break;
    case STATUS_ERROR: *r = 240; *g = 110; *b = 95;  break;
    case STATUS_INFO:
    default:           *r = 200; *g = 208; *b = 220; break;
    }
}
